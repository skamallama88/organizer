...
- [x] Watcher and scanner produce equivalent plans and reports for the same eligible item, with one active processing lease across all triggers.
- [x] Restart recovery moves nonterminal leased work to reconciliation rather than automatically repeating it.
- [x] Immediate runs report snapshot outcomes including executed, skipped, deferred, and outside-snapshot work; unstable items remain deferred with deduplicated diagnostics.

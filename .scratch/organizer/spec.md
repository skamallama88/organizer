...
- Archive output names remove at most one final recognized input archive suffix before appending the requested output extension. For example, `project` becomes `project.zip` or `project.7z`, `project.zip` becomes `project.zip` or `project.7z`, and `bundle.tar.gz` becomes `bundle.tar.gz.zip`; no-overwrite collision handling still applies.
